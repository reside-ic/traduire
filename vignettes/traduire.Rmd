...
* how do we determine what ios the "appropriate" translator object
* how do we determine what language is active for this translation?



To do this, we allow packages (or other simiar code) to "register" a translator, like

```r
traduire::translator_register(resources)
```

where `name` is the key representing your object (more on this later) and `...` are arguments to `traduire::i18n`
...
```
$ curl -H "Accept-Language: fr" http://localhost:8888/hello-world
    ...
    \   ^__^
     \  (oo)\ ________
        (__)\         )\ /\
             ||------w|
             ||      ||
$ curl -H "Accept-Language: en" http://localhost:8888/hello-world
