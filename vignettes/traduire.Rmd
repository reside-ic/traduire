...
The translaion file looks like:
...
There are two ways of achieving this; one is native to `i18next` using its [nesting](https://www.i18next.com/translation-function/nesting) functionality, the other uses [`glue`](https://cran.r-project.org/package=glue) and a custom transformer.
...
This functionality can also be achieved using i18next's ["nesting"](https://www.i18next.com/translation-function/nesting) functionality, with a slight change in the format of the input string, using `$t(...)` instead of `t_(...)`:

```{r}
string2 <- gsub("t_(", "$t(", string, fixed = TRUE)
writeLines(tr$t(string2, language = "fr"))
```

The two methods are very similar - `replace` is possibly more flexible with inputs as it allows strings and character vectors, the native `i18next` method allows the `$t(...)` strings to contain additional options to be passed directly to translator (but the behaviour of the native method depends on the way that the translator has been configured and ity can be disabled with some options).
